			...
			<?php $helper->out($helper->getStorage()->seek('$start_offset'), ';'); ?>
			list(, $trans) = <?php echo $helper->unpackTrans($helper->getStorage()->read('$start_offset', $helper->getTransSize())) ?>;
			...
			if($result) {
				...
				for($idx = 1, $j = 0; ; $j++) {
					...
					// read next trans
					<?php $helper->out($helper->getStorage()->seek($offsetExp), ';'); ?>
					list(, $trans) = <?php echo $helper->unpackTrans($helper->getStorage()->read($offsetExp, $helper->getTransSize())) ?>;
				}
			}
